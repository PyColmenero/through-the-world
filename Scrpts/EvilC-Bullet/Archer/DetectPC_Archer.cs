...
public class DetectPC_Archer : MonoBehaviour
{
    float timer;
    ...
    public float angA;
    public double speedShootTime = 1.2;
    int boos;
    ...
    void Update()
    {
        ...
        if(timer >= speedShootTime/2)
        {
            if(bow != null)
            {
                bow.GetComponent<Animator>().SetBool("shoot", false);
            }
        }
        if(timer >= speedShootTime)
        {
            ...
            if(player.gameObject.transform.position.x < evilX + 30 && player.gameObject.transform.position.x > evilX - 30)
            {
                ...
                {
                    if(bow != null) { bow.GetComponent<Animator>().SetBool("shoot", true); }
